...
func FillThreeSCColumn(SheetName string, file *excelize.File, schedule []Models.ScheduleModel, semester int, rData *Models.RequestData) *excelize.File {
	var Week1 []Models.ScheduleDay
	var Week2 []Models.ScheduleDay
	var Week3 []Models.ScheduleDay
	...
	file.SetCellStyle(SheetName, fmt.Sprintf("%s%d", Models.Columns[0], 9), fmt.Sprintf("%s%d", Models.Columns[8], 9), FullLineStyle)
	...
	file.SetCellValue(SheetName, fmt.Sprintf("%s%d", Models.Columns[8], 9), schedule[2].Group)
	for j := 0; j < 3; j++ {
